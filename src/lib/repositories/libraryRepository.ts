...
const compareString = (left: string, right: string, direction: SortDirection): number => {
    const result = left.localeCompare(right);

    return direction === 'asc' ? result : -result;
};

const compareNumber = (left: number, right: number, direction: SortDirection): number => {
